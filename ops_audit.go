...
// Prune drops all but the newest `keep` events so the in-memory audit trail
// stays bounded over long-running services.
func (a *OpsAudit) Prune(keep int) {
	a.mu.Lock()
	defer a.mu.Unlock()
	if keep < 0 {
		keep = 0
	}
	if len(a.events) <= keep {
		return
	}
	copy(a.events, a.events[len(a.events)-keep:])
	a.events = a.events[:keep]
}
