...
// OpsStatusMapper validates a status against the known status set.
type OpsStatusMapper interface {
	Check(status OpsStatus) error
}

type opsStatusMapper struct {
	allowed map[OpsStatus]bool
}

func (m *opsStatusMapper) Check(status OpsStatus) error {
	if status == "" {
		return nil
	}
	if !m.allowed[status] {
		return fmt.Errorf("%w: unknown status %s", ErrOpsInvalid, status)
	}
	return nil
}
